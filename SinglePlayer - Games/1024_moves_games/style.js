function SfxrP() {
    this.set = function (a) {
        for (var b = 0; b < 24; b++)
            this[String.fromCharCode(97 + b)] = a[b] || 0; this.c < .01 && (this.c = .01);
...
function sy() {
    ...
    this.reset = function () {
        ...
        d = 100 / (a.f * a.f + .001),
            e = 100 / (a.g * a.g + .001),
            f = 1 - a.h * a.h * a.h * .01,
            g = -a.i * a.i * a.i * 1e-6, a.a || (k = .5 - a.n / 2, l = 5e-5 * -a.o),
            h = 1 + a.l * a.l * (a.l > 0 ? -.9 : 10),
            i = 0, j = 1 == a.m ? 0 : (1 - a.m) * (1 - a.m) * 2e4 + 32
    },
        this.totalReset = function () {
            this.reset(); var d = this._p;
            return a = d.b * d.b * 1e5,
                b = d.c * d.c * 1e5,
                c = d.e * d.e * 1e5 + 12, 3 * ((a + b + c) / 3 | 0)
        },
        this.sW = function (m, n) {
            var o = this._p,
                p = 1 != o.s || o.v, q = o.v * o.v * .1, r = 1 + 3e-4 * o.w,
                s = o.s * o.s * o.s * .1, t = 1 + 1e-4 * o.t, u = 1 != o.s,
                v = o.x * o.x, w = o.g, x = o.q || o.r,
                y = o.r * o.r * o.r * .2, z = o.q * o.q * (o.q < 0 ? -1020 : 1020),
                A = o.p ? ((1 - o.p) * (1 - o.p) * 2e4 | 0) + 32 : 0,
                B = o.d,
                C = o.j / 2, D = o.k * o.k * .01,
                E = o.a, F = a, G = 1 / a, H = 1 / b,
                I = 1 / c, J = 5 / (1 + o.u * o.u * 20) * (.01 + s); J > .8 && (J = .8),
                    J = 1 - J; for (var Q, S, U, W, Y, Z,
                        K = !1, L = 0, M = 0, N = 0, O = 0, P = 0, R = 0, T = 0,
                        V = 0, X = 0, $ = 0, _ = new Array(1024), aa = new Array(32),
                        ba = _.length; ba--;)_[ba] = 0;
            for (var ba = aa.length; ba--;)aa[ba] = 2 * Math.random() - 1;
            for (var ba = 0; ba < n; ba++) {
                if (K) return ba;
                if (A && ++X >= A && (X = 0, this.reset()),
                    j && ++i >= j && (j = 0, d *= h),
                    f += g, d *= f, d > e && (d = e, w > 0 && (K = !0)),
                    S = d, C > 0 && ($ += D, S *= 1 + Math.sin($) * C),
                    S |= 0, S < 8 && (S = 8),
                    E || (k += l, k < 0 ? k = 0 : k > .5 && (k = .5)), ++M > F) switch (M = 0, ++L) {
                        case 1: F = b; break;
                        case 2: F = c
                    }switch (L) {
                    case 0: N = M * G; break;
                    case 1: N = 1 + 2 * (1 - M * H) * B;
                        break; case 2: N = 1 - M * I; break;
                    case 3: N = 0, K = !0
                }x && (z += y, U = 0 | z, U < 0 ? U = -U : U > 1023 && (U = 1023)), p && r && (q *= r, q < 1e-5 ? q = 1e-5 : q > .1 && (q = .1)), Z = 0; for (var ca = 8; ca--;) { if (T++, T >= S && (T %= S, 3 == E)) for (var da = aa.length; da--;)aa[da] = 2 * Math.random() - 1; switch (E) { case 0: Y = T / S < k ? .5 : -.5; break; case 1: Y = 1 - T / S * 2; break; case 2: W = T / S, W = 6.28318531 * (W > .5 ? W - 1 : W), Y = 1.27323954 * W + .405284735 * W * W * (W < 0 ? 1 : -1), Y = .225 * ((Y < 0 ? -1 : 1) * Y * Y - Y) + Y; break; case 3: Y = aa[Math.abs(32 * T / S | 0)] }p && (Q = R, s *= t, s < 0 ? s = 0 : s > .1 && (s = .1), u ? (P += (Y - R) * s, P *= J) : (R = Y, P = 0), R += P, O += R - Q, O *= 1 - q, Y = O), x && (_[V % 1024] = Y, Y += _[(V - U + 1024) % 1024], V++), Z += Y } Z *= .125 * N * v, m[ba] = Z >= 1 ? 1 : Z <= -1 ? -1 : Z
            } return n
        }
} var synth = new sy; window.jsfxr = function (a) { window._audioContext = window._audioContext || new AudioContext; var b = window._audioContext; synth._p.set(a); var c = synth.totalReset(), d = c + 1 >> 1 << 1, e = b.createBuffer(1, d, b.sampleRate), f = e.getChannelData(0); 2 * synth.sW(f, c); return e }, window.playSound = function (n) { if (!sound) { return; } jsfxr(n); var o = window._audioContext, e = o.createBufferSource(); e.buffer = jsfxr(n), e.loop = !1, e.connect(o.destination), e.start(o.currentTime); return [e.buffer, o, e] };
...
drawCube = function (obj) {
    ...
    if (type == "ball") {
        ...
        gl.drawElements(gl.TRIANGLES, 6, gl.UNSIGNED_SHORT, 36 + 12);
        gl.uniform1i(shader.isCurrentPlayer, 0);
    } else if (type == "player") {
...
move = function (x, y, p) {
    ...
    if (hauteur > p.z) {
        ...
        if (hauteur - p.z > 2) {
            ...
            if (hauteur > p.z) {
                if (hauteur == p.z + 2) {
                    if (t.o[0].type == 2 && t.o.length == 1) { //Ok 
                        can = 1;
                    }
                }
                if (!can) {
...
function webGLStart() {
    ...
    //CreateD multiple texture for tiles
    for (var j = 0; j < 23; j++) {
    ...
    //Created noise texture for the finish sphere
    s = 256;
    canvas2.width = s;
    canvas2.height = s;
...
initLevels = function () {
    levels = {
        ...
        5: {
            "width": 14, "height": 8, "starts": [{ "x": 1, "y": 4 }], "end": { "x": 11, "y": 4 }, "data": ["00000000000000", "33333333333300", "32222233333300", "33333222333300", "32223222221200", "32223333233300", "33322222233300", "33333333333300"], "objects": [], "triggers": [
                {
                    x: 1,
                    y: 2,
                    type: 1,
        ...
        6: {
            "width": 14, "height": 8, "starts": [{ "x": 1, "y": 4 }], "end": { "x": 11, "y": 4 }, "data": ["00000000000000", "33333333333300", "32222222233300", "33323232333300", "32222232322200", "32223232333300", "33322233333300", "33333333333300"], "objects": [[2, 5], [5, 5]], "triggers": [{
                x: 1,
                y: 2,
                type: 2,
                on: [[8, 4, 3, 2]]
        ...
        9: {
            "width": 14, "height": 8, "starts": [{ "x": 1, "y": 1 }, { "x": 1, "y": 3 }], "end": { "x": 8, "y": 2 }, "data": ["33333333333300", "33333333333300", "33333435533300", "33333333333300", "33333333333300", "00000000000000", "00000000000000", "00000000000000"], "objects": [[1, 2]], triggers: [
                {
                    x: 4,
                    y: 2,
                    type: 2,
        ...
        10: {
            "width": 14, "height": 8, "starts": [{ "x": 2, "y": 6 }, { "x": 10, "y": 2 }], "end": { "x": 5, "y": 4 }, "data": ["00000000000000", "33323332333000", "33323332333000", "33323332333000", "33323532333000", "33323332333000", "33323332333000", "33323332333000"], "objects": [[1, 4], [9, 4]], "triggers": [
                {
                    x: 1,
                    y: 6,
                    type: 2,
        ...
        12: {
            "width": 9, "height": 7, "starts": [{ "x": 5, "y": 3 }], "end": { "x": 7, "y": 3 }, "data": ["111111111", "111111112", "111111012", "111111020", "111111012", "111111112", "111111111"], "objects": [[4, 1, 2], [1, 2], [1, 3], [1, 4], [4, 5, 2]], "triggers": [{
                x: 5,
                y: 5,
                type: 2,
                on: [[5, 5, 1, 2]]
        ...
        13: {
            "width": 8, "height": 7, "starts": [{ "x": 3, "y": 4 }], "end": { "x": 7, "y": 2 }, "data": ["12222200", "22221100", "21111102", "12222101", "21111101", "22222122", "12222222"], "objects": [[2, 2], [2, 4]], "triggers": [{
                x: 5,
                y: 5,
                type: 2,
                on: [[5, 5, 1, 2]]
        ...
        14: {
            "width": 16, "height": 7, "starts": [{ "x": 1, "y": 3 }], "end": { "x": 13, "y": 3 }, "data": ["022222220000200", "0021111122202220", "0221222111222222", "0211111111222212", "0211121112222222", "0221121112002220", "0022222222000200"], "objects": [[2, 2], [4, 3], [7, 3], [6, 4]], "triggers": [{
                x: 2,
                y: 5,
                type: 2,
                on: [[9, 3, 2, 1]]
        ...
        17: {
            "width": 10, "height": 10, "starts": [{ "x": 1, "y": 5 }, { "x": 1, "y": 6 }, { "x": 1, "y": 7 }], "end": { "x": 7, "y": 8 }, "data": ["0000000000", "0000000000", "0000000000", "3333233300", "2111111300", "2111111100", "2111111300", "2111133330", "2111113330", "2222133330"], "objects": [[4, 8]], "triggers": [
                {
                    x: 6,
                    y: 5,
                    type: 2,
